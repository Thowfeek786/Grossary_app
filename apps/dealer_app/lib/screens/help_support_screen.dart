...
class _DealerHelpSupportScreenState extends State<DealerHelpSupportScreen> {
  ...
  Future<void> _makeCall(String phoneNumber) async {
    ...
    try {
      ...
      try {
        ...
        debugPrint('Call launch error: $err');
      }
    }
  }
  ...
  Future<void> _openWhatsApp(String phone, String storeName) async {
    ...
    final nativeUri = Uri.parse('whatsapp://send?phone=$cleanPhone&text=$text');
    final webUri = Uri.parse('https://wa.me/$cleanPhone?text=$text');
    final apiUri = Uri.parse('https://api.whatsapp.com/send?phone=$cleanPhone&text=$text');
    ...
    // 2. Try web wa.me intent
    try {
      final launched = await launchUrl(webUri, mode: LaunchMode.externalApplication);
      if (launched) return;
    } catch (_) {}

    // 3. Fallback to standard platform default browser
    try {
      await launchUrl(apiUri, mode: LaunchMode.platformDefault);
    } catch (err) {
      debugPrint('WhatsApp launch error: $err');
    }
  }
  ...
  Future<void> _sendEmail(String email, String storeName, String storeEmail) async {
    ...
    try {
      ...
      try {
        ...
        debugPrint('Email launch error: $err');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<StoreSettingsModel>(
        stream: SettingsRepository().streamSettings('global'),
        builder: (context, snapshot) {
          final settings = snapshot.data ?? const StoreSettingsModel(id: 'global');
          final helplinePhone = settings.vendorHelplinePhone.isNotEmpty ? settings.vendorHelplinePhone : '1800-889-988';
          final whatsappPhone = settings.whatsappSupportPhone.isNotEmpty ? settings.whatsappSupportPhone : '+919876543210';
          final partnerEmail = settings.vendorSupportEmail.isNotEmpty ? settings.vendorSupportEmail : 'partners@grocerygo.com';
          ...
          return SingleChildScrollView(
            child: Column(
              children: [
                ...
                // 1-Tap Quick Action Buttons (Admin Dynamic)
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          Expanded(
                            child: _ContactActionCard(
                              ...
                              subtitle: helplinePhone,
                              icon: Icons.phone_in_talk_rounded,
                              color: const Color(0xFF059669),
                              onTap: () => _makeCall(helplinePhone),
                            ),
                          ),
                          ...
                          Expanded(
                            child: _ContactActionCard(
                              ...
                              onTap: () => _openWhatsApp(whatsappPhone, storeName),
                            ),
                          ),
                        ],
                      ),
                      ...
                      _ContactActionCard(
                        ...
                        subtitle: partnerEmail,
                        icon: Icons.mail_outline_rounded,
                        color: const Color(0xFF3B82F6),
                        onTap: () => _sendEmail(partnerEmail, storeName, user.email),
                      ),
                    ],
                  ),
                ),
